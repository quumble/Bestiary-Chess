#!/usr/bin/env python3
"""
Artificial Bestiary — GPT-5.4 Cross-Architecture Extension
==========================================================

Runs the bestiary design against the GPT-5.4 family (nano / mini / full),
splitting 2,000 trials evenly across the three models so each (word, condition)
cell is sampled by every model.

Stimulus set:
  20 nonsense words × 10 conditions × 12 trials = 2,400 trials
  800 trials per model (every cell sampled 4× by every model)

Conditions (identical to the Sonnet/Haiku runs):
  neutral
  real / imaginary / type_of  ×  animal / object / idea

Parameters held constant with the Sonnet/Haiku runs:
  temperature = 1.0
...
Usage:
    ...
    # Smoke test (no API):
    python gpt54_cross_arch.py run --dry-run

    # Smoke test (real API, 6 trials):
    python gpt54_cross_arch.py run --limit 6 --out Results/gpt54_smoke.jsonl

    # Full run (~2000 trials):
    python gpt54_cross_arch.py run --out Results/gpt54_results.jsonl

    # Resume after interruption:
    python gpt54_cross_arch.py run --out Results/gpt54_results.jsonl --resume

    # Restrict to one model:
    python gpt54_cross_arch.py run --models gpt-5.4-mini \\
        --out Results/gpt54_mini_only.jsonl

    # Quick analyze:
    python gpt54_cross_arch.py analyze Results/gpt54_results.jsonl \\
        --outdir analysis/gpt54/
"""
...
TRIALS_PER_CELL    = 12       # 20 words × 10 conds × 12 reps = 2400
SEED               = 20260502 # different from Sonnet/Haiku draws

# Twenty-word stimulus set
WORDS = [
    "plindle",
    "frumberlick",
    "skettrop",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_run.add_argument("--out", default="Results/gpt54_results.jsonl")
    p_run.add_argument(
        "--models",
    ...
    p_an.add_argument("--outdir", default="analysis/gpt54")
